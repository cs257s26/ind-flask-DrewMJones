...
def load_data():
    ...
    if os.getcwd().endswith("ProductionCode"):
        ...
        data = load_individual_data("../data/raw/reptiles.csv", data) #../data/raw/reptiles.csv
    else:
        data = load_individual_data("../ind-flask-DrewMJones/ProductionCode/amphibians.csv", data)
        data = load_individual_data("../ind-flask-DrewMJones/ProductionCode/birds.csv", data)
        data = load_individual_data("../ind-flask-DrewMJones/ProductionCode/insects.csv", data)
        data = load_individual_data("../ind-flask-DrewMJones/ProductionCode/mammals.csv", data)
        data = load_individual_data("../ind-flask-DrewMJones/ProductionCode/reptiles.csv", data)
...
def create_leaderboard(common_name_of_animal, data):
    ...
    print_ouput_catch_1, total_output = print_leaders(username_counts, username_key_storage, total_animal_spotted_count, common_name_of_animal) 

    # Return for test purposes
    return username_key_storage, username_counts, total_output
...
def print_leaders(username_counts, username_key_storage, total_animal_spotted_count, common_name_of_animal):
    ...
    print_index = 0
    total_output.append("              LEADERBOARD")
    total_output.append("-------------------------------------")
    total_output.append("      " + str(total_animal_spotted_count) + " " +str(common_name_of_animal) + " sightings")
    total_output.append("------------------------------------")
    for users in username_key_storage:
        print_index += 1
        #lesser_ouput.append("|", str(print_index),"|", "Count:", str(username_counts[users]), "User:", str(users))
        total_output.append("|" + str(print_index) + "|" + "Count:" + str(username_counts[users]) + " User:" + str(users))
        #total_output.append(lesser_ouput)
        lesser_ouput = []
        if print_index == max_user_printout or print_index == len(username_key_storage):
            
            # Return for testing purposes
            return print_index, total_output
            break
